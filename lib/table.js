...
module.exports = React.createClass({
  ...
  initSocket: function() {
    ...
    const socket = IO(this.props.apiUrl+'/'+this.props.pluralName + queryStr);
    ...
    const ret = [
      React.DOM.tr({key: 'header'+Math.random()}, columns.map(col => {
        ...
        const tableHeaderText = TableHeaderText({
          fieldName: col.name,
          updateQuery: this.updateQueryOpts
        });
  ...
  // passed to table headers to allow manipulating the query via sockets
  updateQueryOpts: function(fieldName, opts) {
    const currentOpts = this.state.queryOpts[fieldName];
    let newOpts = _.extend({}, currentOpts, opts);
    newOpts = _.omitBy(newOpts, i => !i);

    let updated = this.state.queryOpts;
    updated[fieldName] = newOpts;

    this.setState({queryOpts: updated});

    // requery socket after query options changed
    this.initSocket();
  },

  buildQueryStr: function() {
    const opts = _.values(this.state.queryOpts);

    const query = _.flatten(opts.map(opt => {
      return _.map(opt, (value, key) => key + "=" + value);
    }));
